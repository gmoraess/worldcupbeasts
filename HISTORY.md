...
- **Power-ups CLICÁVEIS** (pedido do usuário: toque dos jogadores era RNG demais) — o item mostra "clique!" pulsando; clicar pega pro time da CASA (`Match._unhandled_input`, antes dos guards de modo — funciona em Auto e Pro). Buffs (⚡🧲👟) ativam na hora; a 💣 abre **mira em CÂMERA LENTA** (`Engine.time_scale` 0,22, retícula 🎯 segue o mouse via `_process` — que não é afetado pelo time_scale): clique arremessa em arco pro ponto, **botão direito devolve o item** sem gastar. O toque continua valendo pros dois times (corrida pelo item). Guards: `_can_shoot`/`_use_card` bloqueados na mira; troca de velocidade durante a mira só vale ao sair (`_pu_prev_ts`); fim de partida cancela a mira; spawn seguinte segurado. Commit `82c3199`.
